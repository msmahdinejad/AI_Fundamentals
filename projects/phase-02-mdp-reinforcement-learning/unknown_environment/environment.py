...
class WallE_RL:
    ...
    def __load_images(self):
        ...
        for v in [5, 10, 20, 30, 40]:
            try:
                ...
                offset = ((self.__tile_size - overlay_size[0]) // 2.5, (self.__tile_size - overlay_size[1]) // 2.5)
        ...
        try:
            img = pygame.image.load('Env/icons/Hunter.png')
            self.__Hunter_image = pygame.transform.smoothscale(img, (self.__tile_size, self.__tile_size))
        except Exception:
            self.__Hunter_image = pygame.Surface((self.__tile_size, self.__tile_size))
            self.__Hunter_image.fill((255, 100, 0))
    ...
    def __generate_grid(self):
        ...
        while True:
            ...
            for _ in range(self.__num_hunters):
                while True:
                    r, c = random.randint(0, self.__grid_size - 1), random.randint(0, self.__grid_size - 1)
                    if (r, c) not in filled_spaces:
                        grid[r][c] = ('H', 0)
            ...
            if WallE_RL.__is_path_exists(grid=grid, start=(0, 0), goal=(7, 7)):
                break
    ...
    def render(self, screen):
        ...
        y_offset_hunter = self.__tile_size * 0.2
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                if tile_char == 'H':
                    Hunter_dest_y = base_y - y_offset_hunter
                    Hunter_area = None
                    Hunter_img_h = self.__Hunter_image.get_height()
                    if Hunter_dest_y < 0:
                        clip_amount = abs(Hunter_dest_y)
                        Hunter_dest_y = 0
                        Hunter_area = pygame.Rect(0, clip_amount, self.__tile_size, Hunter_img_h - clip_amount)
                    screen.blit(self.__Hunter_image, (base_x, Hunter_dest_y), area=Hunter_area)
