...
class WallE:
    def __init__(self):
        ...
        try:
            ...
            self.__trash_image = pygame.transform.smoothscale(self.__trash_image, (self.__tile_size * 1.1, self.__tile_size * 1.1))
        except Exception:
        ...
        try:
            ...
            self.__flower_image = pygame.transform.smoothscale(self.__flower_image, (self.__tile_size , self.__tile_size))
        except Exception:
    ...
    def render(self, screen):
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                tile_char, tile_val = self.grid[r][c]
    ...
    def __generate_grid(self):
        while True:
            ...
            trash_values = [5, 5, 10, 10, 10, 20, 30, 40]

            random.shuffle(trash_values)

            num_trash = self.__num_trash
            for _ in range(num_trash):
                while True:
                    ...
                    if (r, c) not in filled_spaces:
                        val = trash_values.pop()
                        grid[r][c] = ('S', val)
                        filled_spaces.append((r, c))
                        break

            num_enemies = self.__num_enemies
            ...
            num_buildings = self.__num_buildings
            ...
            if WallE.__is_path_exists(grid=grid, start=(0, 0), goal=(7, 7)):
                break
    ...
    def step(self, action, thresholds):
        actions = {0: (-1, 0), 1: (1, 0), 2: (0, -1), 3: (0, 1)}
        neighbors = {0: [2, 3], 1: [2, 3], 2: [0, 1], 3: [0, 1]}
        ...
        if (0 <= new_row < self.__grid_size and 0 <= new_col < self.__grid_size):
        ...
        current_cell = self.grid[self.__agent_pos[0]][self.__agent_pos[1]]
        tile_char, tile_val = current_cell[0], current_cell[1]
        reward = DEFAULT_REWARD

        if tile_char == 'E':
            reward = ENEMY_REWARD
            self.grid[self.__agent_pos[0]][self.__agent_pos[1]] = ('T', 0)

        elif tile_char == 'S':
            trash_value = tile_val

            current_threshold = thresholds[0]
            for t in thresholds:
                if self.agent_value >= t:
                    current_threshold = t

            if trash_value > current_threshold:
                reward = (current_threshold / trash_value) * GOOD_TRASH_REWARD
                self.grid[self.__agent_pos[0]][self.__agent_pos[1]] = ('T', 0)
            else:
                self.agent_value += trash_value
    ...
    def actions_model(self, reward_map):
        actions_taken = np.full((self.__grid_size, self.__grid_size), -1, dtype=np.int8)
        for x in range(self.__grid_size):
            for y in range(self.__grid_size):
                if self.grid[x][y][0] == 'B':
                    continue
                actions_taken[x, y] = random.choice([0, 1, 2, 3])
            ...
            for col in range(grid_size):
                ...
                for action in range(4):
                    ...
                    if 0 <= next_state[0] < grid_size and 0 <= next_state[1] < grid_size:
                        ...
                        intended_probability = actions_prob[state][action]['intended']
                        transition_table[state][action].append((intended_probability, state, reward_map[row][col]))
                    ...
                    for neighbor_action in neighbors[action]:
                        ...
                        if 0 <= next_state[0] < grid_size and 0 <= next_state[1] < grid_size:
                            ...
                            neighbor_probability = actions_prob[state][action]['neighbor']
                            transition_table[state][action].append((neighbor_probability, state, reward_map[row][col]))
        ...
        for row in range(self.__grid_size):
            for col in range(self.__grid_size):
                ...
                for action in range(4):
                    ...
                    probability_dict[state][action] = {'intended': intended_prob, 'neighbor': neighbor_prob}
